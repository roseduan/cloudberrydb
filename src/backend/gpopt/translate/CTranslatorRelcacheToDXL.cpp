...
CTranslatorRelcacheToDXL::RetrieveStorageTypeForPartitionedTable(Relation rel)
{
	...
	for (int i = 0; i < gpdb::RelationGetPartitionDesc(rel, true)->nparts; ++i)
	{
		...
		if (child_storage == IMDRelation::ErelstorageForeign)
		{
			...
			// Fall back to planner if there is a foreign partition using the cloudberry_fdw
			...
			CWStringConst str_cloudberry_fdw(GPOS_WSZ_LIT("cloudberry_fdw"));
			CAutoMemoryPool amp;
			CMemoryPool *mp = amp.Pmp();
			...
			if (fdw_name_str->Equals(&str_cloudberry_fdw))
			{
				GPOS_DELETE(fdw_name_str);
				GPOS_RAISE(
					gpdxl::ExmaMD, gpdxl::ExmiMDObjUnsupported,
					GPOS_WSZ_LIT(
						"Queries with partitions of cloudberry_fdw are not supported"));
			}
			GPOS_DELETE(fdw_name_str);
