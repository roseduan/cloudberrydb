						 ...
						 Relation target_relation)
{
		...
		relkind == RELKIND_PARTITIONED_TABLE)
	{
		 ...
		 *
		 * Iceberg also needs wholerow for DELETE so that DELETE ... RETURNING
		 * can return the OLD row without an extra single-row re-fetch -- the
		 * Iceberg AM has no random-access tuple_fetch_row_version, and the
		 * row's values are already materialized by the scan.
		 */
		if ((commandType == CMD_UPDATE && RelationIsNonblockRelation(target_relation)) ||
			(commandType == CMD_DELETE && RelationIsIceberg(target_relation)))
		{
			var = makeVar(rtindex,
						  InvalidAttrNumber,
						  RECORDOID,
