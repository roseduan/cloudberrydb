								...
								const SCostingInfo *pci)
{
	...
	CDrvdPropPlan *pdpplanProducer =
		exprhdl.PdpplanCTEProducer(popCTEConsumer->UlCTEId());
	if (nullptr != pdpplanProducer && !pdpplanProducer->FParallelCTEProducer())
		ulWorkers = 1;
							 ...
							 const SCostingInfo *pci)
{
	...
	return CCost((costLocal + costChild)/ulWorkers);
}
							 ...
							 const SCostingInfo *pci)
{
	...
	GPOS_ASSERT(COperator::EopPhysicalInnerHashJoin == op_id ||
				...
				COperator::EopPhysicalParallelLeftAntiSemiHashJoin == op_id ||
