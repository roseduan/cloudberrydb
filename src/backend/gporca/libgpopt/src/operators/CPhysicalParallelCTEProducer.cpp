...
) const
{
	...
	/*
	 * The parallel CTE producer materializes into a SharedTuplestore
	 * that all parallel workers participate in writing.  If the child
	 * delivers Singleton distribution (e.g. Finalize Aggregate), only
	 * one worker would have data and the others contribute nothing.
	 * Consumers then see only 1 sender instead of N.
	 *
	 * Require non-singleton distribution so the optimizer enforces a
	 * motion (Redistribute or Broadcast) to distribute data to all
	 * workers before materialization.
	 */
...
BOOL
CPhysicalParallelCTEProducer::FValidContext(CMemoryPool *,
											COptimizationContext *,
											COptimizationContextArray *pdrgpocChild) const
{
	if (nullptr == pdrgpocChild || 0 == pdrgpocChild->Size())
		return true;

	CCostContext *pccChild = (*pdrgpocChild)[0]->PccBest();
	if (nullptr == pccChild)
		return true;

	/* Accept worker-level distributions directly */
	CDistributionSpec::EDistributionType edtChild =
		pccChild->Pdpplan()->Pds()->Edt();
	if (CDistributionSpec::EdtWorkerRandom == edtChild ||
		CDistributionSpec::EdtHashedWorker == edtChild ||
		CDistributionSpec::EdtReplicatedWorkers == edtChild)
	{
		return true;
	}

	COperator::EOperatorId eopid = pccChild->Pgexpr()->Pop()->Eopid();
	if (COperator::EopPhysicalSerialUnionAll == eopid ||
		COperator::EopPhysicalParallelUnionAll == eopid)
	{
//		ULONG ulWorkers = CUtils::UlExtractWorkersFromGroup(
//			(*pdrgpocChild)[0]->Pgroup(), true /*fStopAtMotion*/);
//		if (ulWorkers > 0)
//		{
//			return true;
//		}
	...
	if (CDistributionSpec::EdtHashed == edtChild ||
		CDistributionSpec::EdtStrictRandom == edtChild ||
		CDistributionSpec::EdtStrictHashed == edtChild)
	{
		ULONG ulWorkers = CUtils::UlExtractWorkersFromGroup(
			(*pdrgpocChild)[0]->Pgroup(), false /*fStopAtMotion*/);
		if (ulWorkers > 0)
		{
			return true;
		}
