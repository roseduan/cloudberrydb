 ...
 * under the License.
 *
 * CPhysicalParallelCTEProducer.h
 *
 * IDENTIFICATION
 *	  src/backend/gporca/libgpopt/include/gpopt/operators/CPhysicalParallelCTEProducer.h
 *
 *-------------------------------------------------------------------------
 */
#ifndef GPOPT_CPhysicalParallelCTEProducer_H
#define GPOPT_CPhysicalParallelCTEProducer_H
...
//
//	@doc:
//		parallel CTE producer operator
//
//---------------------------------------------------------------------------
class CPhysicalParallelCTEProducer : public CPhysical
{
private:
	// cte identifier
	ULONG m_id;
...
public:
	CPhysicalParallelCTEProducer(const CPhysicalParallelCTEProducer &) = delete;

	// ctor
	CPhysicalParallelCTEProducer(CMemoryPool *mp, ULONG id, CColRefArray *colref_array, BOOL *umask, ULONG ulParallelWorkers);

	// dtor
	~CPhysicalParallelCTEProducer() override;

	// ident accessors
	EOperatorId
	Eopid() const override
	{
		return EopPhysicalParallelCTEProducer;
	}

	const CHAR *
	SzId() const override
	{
		return "CPhysicalParallelCTEProducer";
	}
	...
	// reject when child's best plan has parallel degree <= 1
	...
	// conversion function
	static CPhysicalParallelCTEProducer *
	PopConvert(COperator *pop)
	{
		GPOS_ASSERT(nullptr != pop);
		GPOS_ASSERT(EopPhysicalParallelCTEProducer == pop->Eopid());

		return dynamic_cast<CPhysicalParallelCTEProducer *>(pop);
	}

	// debug print
	IOstream &OsPrint(IOstream &) const override;

};	// class CPhysicalParallelCTEProducer

}  // namespace gpopt

#endif	// !GPOPT_CPhysicalParallelCTEProducer_H
