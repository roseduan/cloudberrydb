...
public:
	...
	// Extract worker count from a child group
	// If fStopAtMotion is true, stop searching when a Motion node is found
	static ULONG UlExtractWorkersFromGroup(CGroup *pgroup, BOOL fStopAtMotion = false);

	// Extract worker count from a child group internal
	static ULONG UlExtractWorkersFromGroupInternal(CGroup *pgroup, CBitSet *visited_groups, BOOL fStopAtMotion);
