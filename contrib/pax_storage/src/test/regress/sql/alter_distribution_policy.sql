...
-- Note: distribution is compared by each row's actual segment placement
-- (gp_segment_id together with the row's own columns), not by per-segment row
-- counts. Comparing per-segment counts is fragile for random-to-random
-- redistribution: two independent random distributions can coincidentally yield
-- the identical per-segment count vector and be misreported as "not redistributed".
...
BEGIN
    ...
    before_query := format('SELECT gp_segment_id as segid, x.* FROM %I x', t1);
    after_query := format('SELECT gp_segment_id as segid, x.* FROM %I x', t2);
