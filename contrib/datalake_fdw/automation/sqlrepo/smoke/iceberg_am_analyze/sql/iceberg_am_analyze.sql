...
-- ============================================================
-- T4: Iceberg uses a larger sample target for high-NDV stability.  The
-- session default is restored after ANALYZE; fragment transport remains an
-- internal ExtensibleNode dispatch rather than a GUC.
-- ============================================================
SELECT name FROM pg_settings WHERE name LIKE 'datalake.%analyze%' ORDER BY name;
SET default_statistics_target = 1;
SET datalake.iceberg_analyze_statistics_target = 10;
ANALYZE anlz_t;
SELECT current_setting('default_statistics_target') AS restored_target,
       array_length(histogram_bounds, 1) AS histogram_bounds
  FROM pg_stats WHERE tablename = 'anlz_t' AND attname = 'id';
RESET datalake.iceberg_analyze_statistics_target;
