...
OPTIONS (namespace 'public', table 'deferred_commit_test');
...
-- Test commitRewrite via toolkit function
SET client_min_messages = WARNING;
DO $$
DECLARE
    plan_json   json;
...
BEGIN
    ...
    IF tasks IS NULL OR json_array_length(tasks) = 0 THEN
        RAISE NOTICE 'commitRewrite: no tasks returned from plan_file_groups, skipping';
        RETURN;
    END IF;
