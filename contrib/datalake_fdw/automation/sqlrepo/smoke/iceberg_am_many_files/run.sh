...
#
# Pre-cleans the warehouse/iceberg_am_many_files/ prefix so each run is hermetic
# (DROP TABLE does not purge Iceberg object-store data), then runs the test.
#
# Usage:
#   ./run.sh                     # default database iceberg_am_many_files_smoke
#   DB=mydb ./run.sh             # custom database name
#   CONTAINER=<name> ./run.sh    # custom CBDB container
set -eu

DB="${DB:-iceberg_am_many_files_smoke}"
CONTAINER="${CONTAINER:-hashdata-lightning-umbrella-hashdata-1}"
LAKEHOUSE="${LAKEHOUSE:-lakehouse}"
PGPORT="${PGPORT:-7000}"
HERE_IN_CONTAINER="/workspace/database/contrib/datalake_fdw/automation/sqlrepo/smoke/iceberg_am_many_files"
...
docker exec "$LAKEHOUSE" bash -lc '
    ...
    mc rm --recursive --force local/warehouse/iceberg_am_many_files/ >/dev/null 2>&1
' >/dev/null 2>&1 \
  || docker exec "$CONTAINER" bash -lc 'rm -rf /data/minio/warehouse/iceberg_am_many_files' >/dev/null 2>&1 \
  || true
...
SQL
  ...
  psql -d $DB -v ON_ERROR_STOP=1 -f sql/iceberg_am_many_files.sql 2>&1 | tail -80
"
