...
-- ============================================================
-- Test 13: CREATE ICEBERG TABLE AS SELECT
-- ============================================================
SELECT test_log('Test 13: CREATE ICEBERG TABLE AS SELECT');

CREATE TABLE ct_ctas_src (id bigint, c char(5));
INSERT INTO ct_ctas_src VALUES (1, 'ABCDE'), (2, 'A'), (3, NULL);

CREATE ICEBERG TABLE ct_ctas AS SELECT * FROM ct_ctas_src;
SELECT id, length(c) AS chars, octet_length(c) AS bytes, c IS NULL AS is_null
FROM ct_ctas ORDER BY id;
DROP TABLE ct_ctas;
DROP TABLE ct_ctas_src;

-- ============================================================
-- Test 14: mixed types in one wide table -- CHAR cells must not
-- corrupt adjacent columns.
-- ============================================================
SELECT test_log('Test 14: mixed types in one table');
