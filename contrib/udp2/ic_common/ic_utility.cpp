 ...
 *
 * IDENTIFICATION
 *    contrib/udp2/ic_common/ic_utility.cpp
 *
 *-------------------------------------------------------------------------
 */
 ...
 */
GlobalMotionLayerIPCParam global_param =
	{NULL, /* interconnect_address */
	 -1,   /* Gp_role */
	 0,    /* ic_htab_size */
	 0,    /* segment_number */
	 -1,   /* MyProcPid */
	 ...
	 false,/* MyProcPort */
	 -1,   /* myprocport_sock */
	 8192, /* Gp_max_packet_size */
	 0,    /* Gp_udp_bufsize_k */
	 INTERCONNECT_ADDRESS_TYPE_UNICAST_IC, /* Gp_interconnect_address_type */
	 NULL, /* CheckPostmasterIsAliveCallback */
	 NULL, /* checkInterruptsCallback */
	 NULL, /* simpleFaultInjectorCallback */
	 NULL, /* createOpaqueDataCallback */
	 NULL, /* destroyOpaqueDataCallback */
...
SessionMotionLayerIPCParam session_param =
	{4,    /* Gp_interconnect_queue_depth */
	 ...
	 128,  /* Gp_interconnect_cursor_ic_table_size */
	 5,    /* Gp_interconnect_timer_period */
	 20,   /* Gp_interconnect_timer_checking_period */
	 20,   /* Gp_interconnect_default_rtt */
	 20,   /* Gp_interconnect_min_rto */
	 3600, /* Gp_interconnect_transmit_timeout */
	 100,  /* Gp_interconnect_min_retries_before_timeout */
	 10,   /* Gp_interconnect_debug_retry_interval */
	 false,/* gp_interconnect_full_crc */
	 true, /* gp_interconnect_aggressive_retry */
	 true, /* gp_interconnect_cache_future_packets */
	 false,/* gp_interconnect_log_stats */
	 7200, /* interconnect_setup_timeout */
	 GPVARS_VERBOSITY_TERSE_IC, /* gp_log_interconnect */
	 -1,   /* gp_session_id */
	 INTERCONNECT_FC_METHOD_LOSS_IC, /* Gp_interconnect_fc_method */
	 -1,   /* gp_command_count */
	 0,    /* gp_interconnect_id */
	 IC_WARNING, /* log_min_messages */
	 0,    /* distTransId */

	 UNDEF_SEGMENT,   /* gp_udpic_dropseg */
	 0,    /* gp_udpic_dropacks_percent */
	 0,    /* gp_udpic_dropxmit_percent */
	 0,    /* gp_udpic_fault_inject_percent */
	 0,    /* gp_udpic_fault_inject_bitmap */
	 0,    /* gp_udpic_network_disable_ipv6 */
	};
...
static thread_local std::once_flag Once;
 ...
 * crc32
 */
static uint32 comp_crc32c_sb8(uint32 crc, const void *data, size_t len);
...
#define COMP_CRC32C(crc, data, len) \
	((crc) = comp_crc32c_sb8((crc), (data), (len)))
#ifdef WORDS_BIGENDIAN
#define FIN_CRC32C(crc) ((crc) = ic_bswap32(crc) ^ 0xFFFFFFFF)
#else
#define FIN_CRC32C(crc) ((crc) ^= 0xFFFFFFFF)
#endif

uint32
ComputeCRC(const void *data, size_t len)
{
	uint32 local_crc;

	INIT_CRC32C(local_crc);
	COMP_CRC32C(local_crc, data, len);
 ...
 * systems).
 */
static const uint32 crc32c_table[8][256] = {
...
#ifdef WORDS_BIGENDIAN
#define CRC8(x) crc32c_table[0][((crc >> 24) ^ (x)) & 0xFF] ^ (crc << 8)
#else
#define CRC8(x) crc32c_table[0][(crc ^ (x)) & 0xFF] ^ (crc >> 8)
#endif

uint32
comp_crc32c_sb8(uint32 crc, const void *data, size_t len)
{
	...
	while (len >= 8)
	{
		...
		crc =
			crc32c_table[0][c0] ^ crc32c_table[1][c1] ^
			crc32c_table[2][c2] ^ crc32c_table[3][c3] ^
			crc32c_table[4][c4] ^ crc32c_table[5][c5] ^
			crc32c_table[6][c6] ^ crc32c_table[7][c7];
...
ResetLastError()
{
	ic_error.level = LEVEL_OK;
}

void
SetLastError(ErrorLevel level, const char *msg)
{
	ic_error.level = level;
	strncpy(ic_error.msg, msg, sizeof(ic_error.msg) - 1);
	ic_error.msg[sizeof(ic_error.msg) - 1] = 0;
}
