...
static bool is_remote_gp_cluster(UserMapping *user);
static int get_remote_segment_number(UserMapping *user);
...
postgresBeginForeignScan(ForeignScanState *node, int eflags)
{
	...
	{
		int seg_number = get_remote_segment_number(user);
...
static bool
is_remote_gp_cluster(UserMapping *user)
{
	PGconn     *conn;
	PGresult   *res;
	bool       ret;
	...
	ret = strstr(PQgetvalue(res, 0, 0), "Apache Cloudberry") ? true : false;
...
static int
get_remote_segment_number(UserMapping *user)
{
	PGconn	   *conn;
	PGresult   *res;
	int			size;
